...
public class StreamExercise {
  public static void main(String[] args) {
    ...
    Comparator<Integer> sortedByValueDesc = (n1,n2) -> n1 > n2 ? -1: 1;
    List<Integer> output = numbers.stream().filter(n -> n%2==0).map(n -> n*n).sorted(sortedByValueDesc).collect(Collectors.toList());
    ...
    List<Integer> number2Sorted = numbers2.stream().sorted(sortedByValueDesc).collect(Collectors.toList());
    int max = number2Sorted.get(0);
    int min = number2Sorted.get(number2Sorted.size()-1);
    ...
    List<String> words2Sorted = words2.stream().filter(s -> s.length() > 3).collect(Collectors.toList());
    System.out.println(words2Sorted.size());
    // Output: 4
    ...
    Set<Integer> numbers3Sorted = numbers3.stream().filter(n -> n > 10).collect(Collectors.toSet());
    System.out.println(numbers3Sorted);
    // Output: [15, 20]
    ...
    // new Student("Bob", 75)
    Map<String,Integer> studentClass = new HashMap<>();
    studentClass.put("Alice", 85);
    studentClass.put("Bob", 75);
